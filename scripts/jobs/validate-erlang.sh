...
VALIDATOR_CORRUPTED_EARE="validators/validate_corrupted_eare_erlang.exs"
...
LOG_FILE_CORRUPTED_EARE="$RESULTS_DIR/erlang_corrupted_eare_results.log"
...
echo "Running corrupted EARE shim..."
corrupted_eare_status="failed"
if MIX_ENV=dev mix run "$VALIDATOR_CORRUPTED_EARE" | tee "$LOG_FILE_CORRUPTED_EARE"; then
  corrupted_eare_status="success"
fi

RESULT_FILES=("$RESULTS_DIR/erlang_cbor_status.json" "$RESULTS_DIR/erlang_schema_status.json" "$RESULTS_DIR/erlang_device_desync_status.json" "$RESULTS_DIR/erlang_corrupted_eare_status.json")
TOTAL_TESTS=0
PASSED_TESTS=0
...
if [[ "$cbor_status" == "success" && "$schema_status" == "success" && "$device_desync_status" == "success" && "$corrupted_eare_status" == "success" ]]; then
  JOB_STATUS="success"
else
  JOB_STATUS="failed"
fi
...
cat > "$JOB_STATUS_FILE" <<EOF
{
  ...
  "logs": [
    ...
    "$(basename "$LOG_FILE_CORRUPTED_EARE")",
  ...
  "result_files": ["erlang_cbor_status.json", "erlang_schema_status.json", "erlang_device_desync_status.json", "erlang_corrupted_eare_status.json"]
}
EOF
